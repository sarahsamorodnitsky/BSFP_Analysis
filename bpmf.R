...
model_comparison <- function(mod, p.vec, n, ranks, response, true_params, model_params, s2nX, s2nY, sparsity, nsim, nsample = 2000, n_clust) {
  ...
  # The model options
  models <- c("sJIVE", "BIDIFAC", "BIDIFAC+", "JIVE", "MOFA", "BPMF_Full_Mode", "BPMF_Data_Mode", "BPMF_Full_Mode_No_Scaling", "BPMF_test")
  
  cl <- makeCluster(n_clust)
  registerDoParallel(cl)
  funcs <- c("bpmf_data", "center_data", "bpmf_full_mode", "bpmf_data_mode", "bpmf_test", "get_results", "BIDIFAC",
             "check_coverage", "mse", "ci_width", "data.rearrange", "return_missing",
             "sigma.rmt", "estim_sigma", "softSVD", "frob", "sample2", "logSum",
  ...
  sim_results <- foreach (sim_iter = 1:nsim, .packages = packs, .export = funcs, .verbose = TRUE, .combine = rbind) %dopar% {
    ...
    if (mod == "BPMF_Full_Mode" | mod == "BPMF_Data_Mode" | mod == "BPMF_Full_Mode_No_Scaling" | mod == "BPMF_test") {
