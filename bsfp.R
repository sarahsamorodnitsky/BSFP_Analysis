...
bpmf.predict <- function(bpmf.fit, test_data, Y_test, nninit = TRUE, model_params, sparsity = FALSE, nsample, progress = TRUE, starting_values = NULL) {
  ...
  # Arguments: 
  # 
  # bpmf.fit = results from a training bpmf.fit 
  # test_data = matrix with list entries corresponding to each data source on test samples
  # Y_test = column vector with outcome on test samples or NULL
  # nninit = should the model be initialized with a nuclear norm penalized objective? if FALSE, provide ranks
  # model_params = (error_vars, joint_vars, indiv_vars, beta_vars = NULL, response_vars)
  # nsample = number of Gibbs sampling iterations
  # progress = should the progress of the sampler be displayed?
...
# -----------------------------------------------------------------------------
# BIDIFAC Functions (Credit to Park & Lock (2020))
# -----------------------------------------------------------------------------
      ...
      if (current_param == "EY") {
        ...
        if (!is.null(missingness)) {
          ...
          if (missingness == "missingness_in_response" | missingness == "both") {
            ...
            # Save the results
            results_for_param[[1,1]] <- list(observed = list(avg_coverage = mean(avg_coverage_observed_source[!is.nan(avg_coverage_observed_source)]),
                                                             avg_mse = mean(avg_mse_observed_source[!is.nan(avg_mse_observed_source)]),
                                                             avg_ci_width = mean(avg_ci_width_observed_source[!is.nan(avg_ci_width_observed_source)])),
                                             missing = list(avg_coverage = mean(avg_coverage_missing_source[!is.nan(avg_coverage_missing_source)]),
                                                            avg_mse = mean(avg_mse_missing_source[!is.nan(avg_mse_missing_source)]),
                                                            avg_ci_width = mean(avg_ci_width_missing_source[!is.nan(avg_ci_width_missing_source)])))
...
imputation_simulation <- function(mod, p.vec, n, ranks, response, true_params, model_params = NULL, s2nX, s2nY, nsim, nsample = 2000, n_clust, missing_data_type, prop_missing) {
  ...
  # for (sim_iter in 1:nsim) {
    ...
    if (mod == "BSFP") {
      ...
      imputed_values_list <- lapply(1:q, function(s) {
      ...
      # Create a matrix of imputed values
      imputed_values_list_mat <- lapply(1:q, function(s) {
        do.call(rbind, imputed_values_list[[s]])
      })
      
      # Calculate credible intervals for each imputed values
      imputed_values_cis <- lapply(1:q, function(s) {
        apply(imputed_values_list_mat[[s]], 2, function(col) {
      ...
      imputed_values <- lapply(1:q, function(s) colMeans(imputed_values_list_mat[[s]]))
